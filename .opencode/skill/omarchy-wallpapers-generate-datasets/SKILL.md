...
- **`preview` + `image`:** theme-level entries (`datasets.json`) and per-collection entries (`collections.json`) expose a random `preview` (640x360) together with `image`, the URL of the full 2K image corresponding to the same wallpaper (lowest available resolution if 2K is missing). Masters have neither, since they have no previews. At the theme level the random pick is overridden when the theme sets an explicit `image` in `datasets/config.json` (base name, filename or repo-relative path): the script resolves it and builds the full preview/image URLs.
...
- If a new theme or collection was added, update `datasets/config.json` first (title/description and, for themes, the 5-color `palette` and the optional cover `image`) — it is the static source of truth and protected from stale-folder cleanup.
