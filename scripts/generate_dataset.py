...
def configured_url_base():
    """Return the base URL for generated links from datasets/config.json.

    The static config is the source of truth: its `url_base` (e.g. the
    CloudFront distribution) is used for every absolute URL, falling back to
    GitHub raw when unset."""
    try:
        with open(CONFIG_PATH, encoding="utf-8") as f:
            base = json.load(f).get("url_base", "")
    except (OSError, ValueError):
        base = ""
    return base.rstrip("/")


RAW_BASE = configured_url_base() or f"https://raw.githubusercontent.com/{REPO}/{BRANCH}"
...
def collections_in(base_dir):
    """Collections are the subdirectories of a theme/masters folder."""
...
def scan_theme(theme_dir):
    entries = []
    for collection in collections_in(theme_dir):
        collection_dir = os.path.join(theme_dir, collection)
        if not os.path.isdir(collection_dir):
            continue
...
def scan_masters():
    entries = []
    for collection in collections_in(MASTERS_COLORS_DIR):
...
def resolve_cover(theme, ref):
    ...
    for collection in collections_in(os.path.join(IMAGES_DIR, theme)):
        image_dir = os.path.join(IMAGES_DIR, theme, collection)
