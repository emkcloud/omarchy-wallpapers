...
`datasets/config.json` is the **static** source of truth for human-readable metadata (theme `title`/`description`/`palette` and collection `title`/`description` in English). The theme `palette` is an array of 5 hex colors, always in this order: `dark_background` (darkest base), `background` (base), `muted` (surface/gray), `foreground` (text), `accent` (signature color), taken from the theme's official `colors.toml`. A theme may also set an optional `image` (the base name of one of its wallpapers, e.g. `omarchy-country-IT-Italy`) to pin its cover in `datasets.json`; when absent, the cover is a random preview. `generate_dataset.py` reads it and injects the descriptions and the palette into `datasets.json`, `catalog.json` and `collections.json`. **Whenever a new theme or collection is added, update `datasets/config.json` first** (a missing entry just means no description/palette); it is protected from the stale-folder cleanup.
...
- **Never delete, move, rename or overwrite user files.** The only folder you own is `working/generate-temp/`. Anything else (`working/`, `images/`, `masters/`, `previews/`, `datasets/`, scripts, docs, configs) is the user's: change it only with an explicit request and never remove it.
- **Back up before modifying.** Before editing an existing file (or a batch of files), save a timestamped copy outside the repository, e.g. `emkcloud/omarchy-wallpapers-backups/<name>-YYYYmmdd-HHMMSS/`, and tell the user where it is. Prefer creating a new file over overwriting one.
- **Plan first, act after approval.** For anything involving more than a couple of files, deletions, moves, or bulk operations, present the plan and wait for the user's OK before executing.
- **`working/` is not disposable.** It is gitignored, so it has no history and cannot be restored: treat it as important user data.
- **Git operations require explicit approval:** never run `git commit`, `git push`, `git tag`, `git reset`, `git checkout`, `git restore`, `git clean`, or `git stash` unless the user explicitly asks for that exact action. Do not commit or push "to be helpful".
- **Never revert with `git checkout`/`git restore`:** to undo a temporary change, edit the file back in a targeted way (or ask first). A `git checkout <file>` discards the user's uncommitted edits and is forbidden without explicit approval.
